        ...
        <tbody>
          @forelse($orderpackages as $orderpackage)
            <tr>
              ...
              <td><a href="/images/printscreen/{{ $orderpackage->printscreen }}" target="_blank"> Printscreen</a></td>
              <td>
                @if ($orderpackage->status == 2)
                  <button class="btn btn-danger btn-sm m-0">DESACTIVATED</button>
                @elseif($orderpackage->status == 1)
                  <button class="btn btn-success btn-sm m-0">ACTIVATED</button>
                @else
